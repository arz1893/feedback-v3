<template>
    <div>
        <span class="text-red" v-show="serviceCategories.length === 0">Sorry you haven't add some category to this service, please add it first on master data</span>

        <div v-if="serviceCategories.length > 0">
            ...
            <div class="alert alert-success" role="alert" id="alert_success" v-show="showAlert" @click="showAlert = false">
                ...
                <strong>Info!</strong> Feedback has been added, please look at <a role="button" class="alert-link">feedback service list</a> to see all added feedback from customers
            </div>
            ...
            <div v-show="showForm === false && loadingState === false">
                <button v-for="serviceCategory in serviceCategories" class="btn btn-lg btn-info btn3d"
                        :node_id="serviceCategory.id"
                        :data-service_id="serviceCategory.serviceId"
                        :data-name="serviceCategory.name" @click="serviceCategoryAction(serviceCategory)">
                    <i class="ion ion-navigate" aria-hidden="true"></i> {{ serviceCategory.name }}
                    <span class="badge bg-teal" v-if="serviceCategory.childs > 0">{{ serviceCategory.childs }}</span>
                </button>
            </div>
        </div> <br>
        ...
        <div class="row">
            <div class="col-lg-6" v-show="showForm">
                <transition name="fadeDown">
                    <div class="panel panel-danger" id="panel_service">
                        <div class="panel-heading">
                            <h4>Add Feedback to : {{ serviceCategory.name }}</h4>
                        </div>
                        <div class="panel-body">
                            ...
                            <div class="error">
                                ...
                                <input type="radio" name="customer_rating" id="radio_dissatisfied" class="invisible" value="1" v-model="feedbackService.rating" v-validate="'required|in:1,2,3'"/>
                            ...
                            <div class="form-group" :class="{'has-error': errors.has('feedback')}">
                                ...
                                <textarea class="form-control" name="feedback" id="feedback" placeholder="Please enter customer's feedback" rows="6" v-model="feedbackService.feedback" v-validate="'required'"></textarea>
...
<script>
    ...
    export default {
        name: "feedback-show",
        props: ['service_id', 'tenant_id', 'syscreator'],
        data() {
            return {
                serviceCategories: [],
                serviceCategory: [],
        ...
        methods: {
            ...
            serviceCategoryAction: function (selectedCategory) {
                let vm = this;
                if(selectedCategory.childs === 0) {
                    vm.serviceCategory = selectedCategory;
                    vm.showForm = true;
                    vm.showBack = true;
            ...
            getParentNodes: function() {
                let vm = this;
                const url = window.location.protocol + "//" + window.location.host + "/" + 'api/service_category/' + vm.previousNode.id + '/get-parent-nodes';
                    ...
                    function sendRequest() {
                        axios.get(url).then(response => {
                            ...
                            for(let index in response.data.parentNodes) {
                                vm.serviceCategories.push(response.data.parentNodes[index]);
                            }
            ...
            getChildNodes: function (parent_id) {
                let vm = this;
                const url = window.location.protocol + "//" + window.location.host + "/" + 'api/service_category/' + parent_id + '/get-child-nodes';
                vm.loadingState = true;
                function sendRequest() {
                    axios.get(url).then(response => {
                        vm.serviceCategories = response.data.data;
            ...
            goBack: function () {
                ...
                vm.feedbackService.rating = '';
                vm.feedbackService.feedback = '';
                vm.feedbackService.fileName = '';
                vm.feedbackService.image = '';
                vm.feedbackService.need_call = 0;
                vm.feedbackService.is_urgent = 0;
            ...
            validateBeforeSubmit: function () {
                ...
                this.$validator.validateAll().then((result) => {
                    if (result) {
                        const url = window.location.protocol + "//" + window.location.host + "/" + 'api/feedback_service/' + vm.tenant_id + '/add-feedback-service';
                        axios.post(url, {
                            feedbackService: vm.feedbackService,
                            serviceId: vm.service_id,
                            serviceCategoryId: vm.serviceCategory.id,
